...
""")
    # Basic
    parser.add_argument("--input_file", help="Path to input alignment file or directory containing multiple files", default=None)
    parser.add_argument("--GB_input", help="Path to a dataframe containing GenBank accession numbers (CSV/TSV). If provided, sequences will be downloaded from GenBank and aligned using MAFFT before preprocessing.", default=None)
    ...
    # Trimming
    parser.add_argument("--orphan_method", help="Method to trim orphan nucleotides. Options: 'none' (default), 'auto' (define a threshold using percentiles), 'semi' (define a threshold using an integer)", choices=["auto", "semi"], default=None)
    parser.add_argument("--orphan_threshold", type=int, help="Threshold integer if orphan_method='semi' (default: 10)", default=10)
    parser.add_argument("--percentile", type=float, help="Percentile if orphan_method='auto' (default: 25).", default=25.0)
    ...
    parser.add_argument("--internal_leaves", help="Sequence names to apply the method to. Use 'all' or comma-separated list (e.g., seq1,seq2)", type=parse_internal_leaves, default="all")
    parser.add_argument("--internal_threshold", type=int, help="Threshold for internal gaps if method='semi'", default=None)

    # Partitioning
    parser.add_argument("--partitioning_round", type=parse_partitioning_round, help="Round of successive partitioning. If 0, partitioning is not performed.", default=0)
