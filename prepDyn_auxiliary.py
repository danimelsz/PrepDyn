...
def GB2MSA(input_file, 
    ...
    4. Replaces terminal '?' in sequences with '-'.
    5. Deletes intermediate files ending with '_aligned.fasta'.
    6. Optionally logs wall clock and CPU time to a log file named '<output_prefix>_log.txt'.
    """
    ...
    # Step 4: Replace terminal '?' with '-' in each sequence
    for cleaned_file in cleaned_files:
        records = list(SeqIO.parse(cleaned_file, "fasta"))
        updated_records = []
        for record in records:
    ...
    # Step 5: Delete intermediate *_aligned.fasta files
    for aligned_file in aligned_files:
        if aligned_file.endswith("_aligned.fasta") and os.path.exists(aligned_file):
            os.remove(aligned_file)

    # Step 6: Log timing if requested
    if log:
        end_wall = time.time()
